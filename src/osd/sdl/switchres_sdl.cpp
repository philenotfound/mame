...
bool switchres_modeline_setup(running_machine &machine)
{
	...
	// Disable -syncrefresh if our vfreq is scaled or too off, or we don't have DRI access
	set_option_osd(machine, OPTION_SYNCREFRESH, !(fd == 0 || best_mode->result.weight & R_V_FREQ_OFF || best_mode->result.v_scale > 1) || black_frame_insertion);
